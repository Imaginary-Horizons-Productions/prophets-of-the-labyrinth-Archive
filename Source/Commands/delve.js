...
const { prerollBoss, labyrinthExists, defaultLabyrinths } = require('../labyrinths/_labyrinthDictionary.js');
...
const mainId = "delve";
const options = [
	{ type: "String", name: "labyrinth", description: "The value to base the run's random events on", required: true, autocomplete: true, choices: [] },
	{ type: "String", name: "seed", description: "The value to base the run's random events on", required: false, choices: [] }
];
module.exports = new Command(mainId, "Start a new adventure", false, false, options);
...
module.exports.execute = (interaction) => {
	...
	const adventure = new Adventure(interaction.options.getString(options[1].name), interaction.guildId).generateRNTable();
	// roll bosses
